            ...
            if selected_method == "MLEM":
                ...
                while ep < epochs:
                    ...
                    if (ep==0):
                        ...
                        # self.text_label.setVisible(True)
                        # plt.plot(PSNR_val[19:-1])
                        # plt.title(f'PSNR {round(PSNR_val[-1],3)}, iter {ep+1}/{epochs}')
                        # plt.savefig(f"PSNR_plot_{core_iteration}its_{ep + 1}epochs.png")
                        # print(PSNR_val)
                        img1 = cv2.resize(255 * torch_to_np(recon), [369, 369])
                        cv2.imwrite("hello_world.png", img1)
                        time.sleep(1)
